...
var hModal = function(content, settings) {
  ...
  this.showSlideOut = function(html, options) {
    ...
    $(obj.element)
      .removeClass("hmodal-popup")
      .removeClass("hmodal-modal")
      .removeClass("hmodal-popup-scroll-inner")
      .removeClass("hmodal-popup-full-screen");

    $(obj.element).addClass("hmodal-slideout");
  ...
  this.showPopUp = function(html, options) {
    ...
    $(obj.element)
      .removeClass("hmodal-slideout")
      .removeClass("hmodal-modal")
      .removeClass("hmodal-slideout-right")
      .removeClass("hmodal-slideout-left")
      .removeClass("hmodal-overlay-close")
      .removeClass("close-button-outside-right")
      .removeClass("hmodal-popup-full-screen");

    $(obj.element).addClass("hmodal-popup");
  ...
  this.setUp = function(options) {
    ...
    if (options && textIsInitialized(options.customClass)) {
      $(obj.element).addClass(options.customClass);
    } else {
      $(obj.element).removeClass(options.customClass);
    }

    //checks and sets overlay color
    if (options && options.overlayColor === "light") {
      $(obj.element).addClass("overlay-light");
    } else {
      $(obj.element).removeClass("overlay-light");
    }

    //checks and sets if overlay can be clicked to close it
    if (
      options &&
      !(options.overlayOnClickClose === false) &&
      obj.type != "modal"
    ) {
      $(obj.element).addClass("hmodal-overlay-close");
      $("#" + obj.id + " .hmodal-overlay").unbind(); //must unbind because otherwise you get the close function running multiple times on click
      $("#" + obj.id + ".hmodal-overlay-close .hmodal-overlay").bind(
        "click",
        function(e) {
          e.preventDefault();
          obj.close();
      ...
      obj.type === "modal" &&
      options &&
      options.overlayOnClickClose === true
    ) {
      $(obj.element).addClass("hmodal-overlay-close");
      $("#" + obj.id + " .hmodal-overlay").unbind(); //must unbind because otherwise you get the close function running multiple times on click
      $("#" + obj.id + ".hmodal-overlay-close .hmodal-overlay").bind(
      ...
      $(obj.element).removeClass("hmodal-overlay-close");
  ...
  this.open = function() {
    ...
    $(obj.element)
      .addClass("hmodal-open-container")
      .trigger("open", [obj.id]);
    ...
    if (
      ...
      $(".hmodal-popup").length > 0 &&
      $(".hmodal-popup-scroll-inner").length == 0
    ) {
  ...
  this.close = function() {
    ...
    $(obj.element)
      .trigger("close", [obj.id])
      .removeClass("hmodal-open-container");
  ...
  this.setHeader = function(options) {
    // returns string html of the header
    if (options.header) {
      $(obj.element).removeClass("hmodal-hide-fixed-header");
    ...
    $(obj.element).addClass("hmodal-hide-fixed-header");
  };

  this.setBody = function(html, options) {
    if (textIsInitialized(html)) {
      //runs if html isnt blank
      $("#" + obj.id + " .hmodal-content-main").html(html);
    } else if (!textIsInitialized(html) && obj.content) {
      //shows obj content as last resort
  ...
  this.setFooter = function(options) {
    ...
    if (
      !textIsInitialized(options.footer) &&
      (!options.buttons || !options.buttons[0]) &&
      obj.type != "modal"
    ) {
      $(obj.element).addClass("hmodal-hide-fixed-footer");
    }
  ...
  this.setButtons = function(options) {
    if (options.buttons) {
      options.buttons.forEach(function(button, index, array) {
        ...
        newButton.unbind();
        if (!button.callback) {
          //if a callback function doesn't exist, defaults to the close function
  ...
  this.setCloseButton = function(options) {
    ...
    $(obj.element).removeClass(
      "close-button-inside-right close-button-inside-left close-button-outside-right close-button-outside-left hmodal-hide-close-button"
    ); //fresh start to close button
    //binds close to close hmodal button

    if (options.closeButton) {
      //prevents crash if there are no options set
      //popup settings
      if (options.closeButton.show === false) {
        $(obj.element).addClass("hmodal-hide-close-button");
        return;
      }

      if (obj.type == "popup") {
        if (!(options.closeButton.inside === false)) {
          //if it's definitely inside
          if (!(options.closeButton.right === false)) {
            //if it's definitely to the right
            $(obj.element).addClass("close-button-inside-right");
          } else {
            $(obj.element).addClass("close-button-inside-left");
          }
        } else {
          //if it's not inside
          if (!(options.closeButton.right === false)) {
            //if it's definitely to the right
            $(obj.element).addClass("close-button-outside-right");
          } else {
            $(obj.element).addClass("close-button-outside-left");
          }
        }
      }
      //slideout settings
